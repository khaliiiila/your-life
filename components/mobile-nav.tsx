...
export function MobileNav() {
  ...
  return <div className="mobile-brand"><button className="icon-button" aria-label="Buka navigasi" aria-expanded={open} onClick={() => setOpen(true)}><Menu size={20} /></button><span>dompetku</span>{open && <><button className="drawer-backdrop" aria-label="Tutup navigasi" onClick={() => setOpen(false)} /><nav className="mobile-drawer" aria-label="Navigasi mobile"><div className="drawer-header"><strong>dompetku</strong><button ref={closeRef} className="icon-button" aria-label="Tutup navigasi" onClick={() => setOpen(false)}><X size={20} /></button></div>{links.map(([href, label, Icon]) => <Link className="nav-item" href={href} key={href} onClick={() => setOpen(false)}><Icon size={18} />{label}</Link>)}</nav></>}</div>;
}
