...
export async function getDashboardData() {
  ...
  const [walletsResult,flowResult,transactionsResult,upcomingResult,investmentsResult,debtsResult,topWalletsResult,topCategoryResult,daily]=await Promise.all([
    ...
    query(`SELECT COALESCE(SUM(amount) FILTER (WHERE type='income' AND to_char(date,'YYYY-MM')=$1),0) income,COALESCE(SUM(amount) FILTER (WHERE type='expense' AND to_char(date,'YYYY-MM')=$1 AND category<>'transfer'),0) expenses,COALESCE(SUM(amount) FILTER (WHERE type='income' AND to_char(date,'YYYY-MM')=$2),0) "prevIncome",COALESCE(SUM(amount) FILTER (WHERE type='expense' AND to_char(date,'YYYY-MM')=$2 AND category<>'transfer'),0) "prevExpenses" FROM transactions`,[month,shiftMonth(month,-1)]),
    ...
    query("SELECT COALESCE(SUM(d.principal_amount-COALESCE(p.paid,0)) FILTER (WHERE d.direction='owed_by_me'),0) owed,COALESCE(SUM(d.principal_amount-COALESCE(p.paid,0)) FILTER (WHERE d.direction='owed_to_me'),0) receivable FROM debts d LEFT JOIN (SELECT debt_id,SUM(amount) paid FROM debt_payments GROUP BY debt_id) p ON p.debt_id=d.id WHERE d.status='active'"),
    query("SELECT w.id,w.name,w.type,COUNT(*) cnt,COALESCE(SUM(t.amount),0) total FROM transactions t JOIN wallets w ON w.id=t.wallet_id WHERE t.type='expense' AND t.category<>'transfer' GROUP BY w.id,w.name,w.type ORDER BY cnt DESC,total DESC LIMIT 5"),
    query("SELECT category,SUM(amount) total FROM transactions WHERE type='expense' AND category<>'transfer' AND to_char(date,'YYYY-MM')=$1 GROUP BY category ORDER BY total DESC LIMIT 1",[month]),getDailyExpenses()]);
  const wallets=walletsResult.rows.map(r=>numbers(r,["balance"])); const flow=numbers(flowResult.rows[0],["income","expenses","prevIncome","prevExpenses"]); const transactions=transactionsResult.rows.map(r=>numbers(r,["amount"])); const upcoming=upcomingResult.rows.map(r=>numbers(r,["amount"])); const investments=numbers(investmentsResult.rows[0],["value","gain"]); const debts=numbers(debtsResult.rows[0],["owed","receivable"]); const totalBalance=wallets.reduce((sum,w)=>sum+Number(w.balance),0); const netWorth=totalBalance+Number(investments.value)+Number(debts.receivable)-Number(debts.owed);
  const topCategoryRow=topCategoryResult.rows[0];
  return {wallets,flow,transactions,upcoming,investments,debts,topWallets:topWalletsResult.rows.map(r=>numbers(r,["cnt","total"])),topCategory:topCategoryRow?{category:topCategoryRow.category,total:Number(topCategoryRow.total)}:null,totalBalance,netWorth,month,daily};
}
