...
export async function listWishlists(pagination: Pagination) {
  ...
  const rows=(await query("SELECT * FROM wishlists WHERE status='active' ORDER BY CASE priority WHEN 'high' THEN 0 WHEN 'medium' THEN 1 ELSE 2 END,target_date IS NULL,target_date,name LIMIT $1 OFFSET $2",[pagination.pageSize,pagination.offset])).rows.map(r=>numbers(r,["target_amount","saved_amount"]));
  const summary=numbers((await query<{target:string;saved:string;count:string}>("SELECT COALESCE(SUM(target_amount),0) target,COALESCE(SUM(saved_amount),0) saved,COUNT(*) count FROM wishlists WHERE status='active'")).rows[0],["target","saved","count"]);
