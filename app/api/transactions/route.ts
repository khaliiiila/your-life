...
export function GET(request: Request) {
  ...
  const result = listTransactions({ type: url.searchParams.get("type") || undefined, walletId: url.searchParams.get("walletId") || undefined, category: url.searchParams.get("category") || undefined, from: url.searchParams.get("from") || undefined, to: url.searchParams.get("to") || undefined }, pagination);
...
export async function POST(request: Request) {
  try {
    ...
    const id = createTransaction(body);
