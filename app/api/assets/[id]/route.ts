...
export async function PATCH(request: Request, context: Context) {
  try {
    ...
    updateAssetValue(id, body.currentValue, body.valuationDate); return NextResponse.json({ ok: true });
...
export async function DELETE(_request: Request, context: Context) {
  try { deleteAsset((await context.params).id); return NextResponse.json({ ok: true }); }
