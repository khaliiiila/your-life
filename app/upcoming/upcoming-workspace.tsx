...
export function UpcomingWorkspace() {
  ...
  async function load() {
    ...
    try {
      const [expenseResponse, walletResponse] = await Promise.all([fetch("/api/upcoming-expenses"), fetch("/api/wallets")]);
      ...
      setExpenses(expenseData.expenses); setWallets(walletData.wallets);
