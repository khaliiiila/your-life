...
export function WalletsWorkspace() {
  ...
  async function create(event: FormEvent) { 
    ...
    if (!Number.isInteger(startingBalance)) { 
      setMessage("Saldo awal harus berupa angka bulat."); 
      addToast("Saldo awal harus angka bulat", "error");
      setSaving(false); 
    ...
    if (!form.name.trim()) {
      setMessage("Nama wallet tidak boleh kosong");
      addToast("Nama wallet wajib diisi", "error");
      return;
    }
    ...
    try { 
      const response = await fetch("/api/wallets", { 
        ...
        body: JSON.stringify({ ...form, startingBalance }) 
      }); 
      const result = await response.json(); 
      if (!response.ok) throw new Error(result.error); 
      
      setForm(blank); 
      const successMsg = "Wallet berhasil ditambahkan.";
      setMessage(successMsg);
      addToast(successMsg, "success");
      
      await load(); 
    } catch (createError) { 
      const errorMsg = createError instanceof Error ? createError.message : "Wallet tidak dapat disimpan.";
      setMessage(errorMsg);
  ...
  return <><ToastContainer toasts={toasts} onClose={removeToast} /><a className="skip-link" href="#wallet-list">Lewati ke daftar wallet</a><header className="mobile-page-bar"><MobileNav /></header><div className="content transaction-page"><div className="page-heading"><div><p className="eyebrow">SUMBER DANA</p><h1>Wallet</h1><p className="muted">Kelola rekening, uang tunai, e-wallet, dan kreditmu.</p></div></div><div className="wallet-total card"><span>Total seluruh wallet</span><strong>{idr.format(total)}</strong><small>{wallets.length} wallet aktif</small></div><div className="transaction-layout wallet-layout">
    <section className="card form-card"><div className="form-title-icon"><WalletCards size={20} /></div><h2>Tambah wallet</h2><p className="muted form-intro">Saldo awal menjadi dasar perhitungan sebelum transaksi.</p><form onSubmit={create}>
      ...
      <Field label="Saldo awal" id="wallet-balance"><input ref={balanceRef} id="wallet-balance" type="number" inputMode="numeric" step="1" value={form.startingBalance} onChange={(event) => setForm({ ...form, startingBalance: event.target.value })} required /></Field>
      {message && <div className="form-message success" role="status"><span>{message}</span><button type="button" aria-label="Tutup pesan" onClick={() => setMessage("")}><X size={16} /></button></div>}
      <button className="button primary submit-button" disabled={saving}>{saving ? <LoaderCircle className="spin" size={17} /> : <Plus size={17} />}Tambah wallet</button>
    </form></section>
    <section className="card list-card" id="wallet-list">
